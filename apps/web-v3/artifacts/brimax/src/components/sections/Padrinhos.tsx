...
function PersonCard({ person, index, total }: { person: Person; index: number; total: number }) {
  ...
  const eyebrow = person.isFamily
    ? `Família · ${person.role}`
    : `Padrinhos · ${String(index + 1).padStart(2, "0")} / ${String(total).padStart(2, "0")}`;
  ...
  return (
    ...
    >
      <div className="relative aspect-[4/3] overflow-hidden bg-[#f7f3ec]">
        ...
        />
        {person.isFamily && (
          <span className="absolute left-4 top-4 rounded-full bg-[#d6ae64] px-3 py-1 text-[10px] font-medium uppercase tracking-[0.2em] text-white">
            Família
          </span>
        )}
      </div>
      ...
      <div className="flex flex-1 flex-col p-6 md:p-7">
        <p className="padrinhos-v4-eyebrow text-xs uppercase text-foreground/55">{eyebrow}</p>
        <h3
          id={`person-name-${person.id}`}
          className="padrinhos-v4-display mt-3 text-2xl leading-tight tracking-tight md:text-[1.75rem]"
        >
          {person.name}
        </h3>
      </div>
        ...
        >
          <div className="flex items-stretch gap-5 md:gap-7">
            {people.map((p, index) => (
              <motion.div
                key={p.id}
                className="flex min-w-0 flex-[0_0_85%] md:flex-[0_0_55%] lg:flex-[0_0_22rem]"
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ duration: 0.6, delay: Math.min(index, 6) * 0.06 }}
              >
                <PersonCard person={p} index={index} total={people.length} />
              </motion.div>
            ))}
          </div>
        </div>
