...
- **Dev server.** `pnpm dev:web` (port 5173) loads the repo-root `.env.dev` by default; production requires the explicit override `BRIMAX_ENV_FILE=.env BRIMAX_LOCAL_ALLOW_PROD=true pnpm dev:web`. [vite.config.ts](vite.config.ts) honors `PORT`, `BASE_PATH`, and `BUILD_OUT_DIR` env overrides. In dev it also proxies `/media/*` to the deployed CDN (default `https://dev.brimax.life`, override with `MEDIA_PROXY_TARGET`) so images/video resolve locally, and **read-only** proxies `/api/*` (GET/HEAD only) to the deployed HTTP API (default `https://api.dev.brimax.life`, override with `API_PROXY_TARGET`) so data like guest messages loads without hitting the prod CORS allowlist. Writes are blocked locally (the dev server answers 404), so local dev can never mutate production. Both proxies are dev-only and never enter production builds. `guest-messages-api.ts` and `gifts-api.ts` use the `/api` base in dev (`import.meta.env.DEV`) so their reads load locally; other API libs (e.g. `rsvp-api.ts`) call `VITE_API_URL` directly. Note: the RSVP invitation lookup runs server-side Turnstile (`verifyTurnstile`), so it can't be read locally through the proxy — the production site key does not accept `localhost`, so no valid token is produced — and is intentionally left un-proxied. `apps/web/.env` is reference-only and is not part of the supported runtime flow.
