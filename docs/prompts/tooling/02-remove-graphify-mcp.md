...
1. Record `git status --short` and preserve every pre-existing change, including Prompt 1's unstaged edits.
2. Inventory Graphify MCP references and executable surfaces in the repository, `PATH`, `uv tool list`, user executable directories, Codex profiles/configuration, Claude Code configuration/skills/hooks, and Kimi Code configuration/skills/hooks. Do not display secrets.
3. Back up any personal configuration that requires editing under `${XDG_CACHE_HOME:-$HOME/.cache}/brimax-life-tooling-removal-backups/graphify-mcp/`, using a timestamped directory and restrictive permissions.
4. Resolve external benchmark paths from trusted metadata where available, canonicalize them, and verify they are descendants of the dedicated Graphify benchmark cache before deletion. Never delete an entire general cache/configuration root or use an unresolved path/glob.
5. If existing scoring artifacts disagree, required source artifacts are missing, or a personal configuration cannot be safely parsed, stop the affected operation and report the discrepancy. Do not invent audit values.
...
3. Preserve and, if necessary, clarify `benchmarks/graphify-context/results.md` as the human-readable final pilot report. It may retain historical references to Graphify MCP and Serena.
4. Build `benchmarks/graphify-context/pilot-audit.json` from the existing frozen external scoring and validation artifacts before deleting them. It must be valid, compact JSON and include enough provenance to audit the final selection: schema/version marker, pilot date or run identity, arms/tasks measured, aggregate accuracy/quality results, token/cost/wall-time comparisons, MCP overhead/failure results, final decision, limitations, and hashes of source scoring artifacts when available. Preserve numbers exactly; distinguish independently scored results rather than blending them. Omit raw model answers, secrets, and machine-specific personal paths.
5. Replace `benchmarks/graphify-context/README.md` with a short historical archive notice explaining that only `results.md` and `pilot-audit.json` are retained, the executable pilot is retired, and these records are not current tooling instructions.
6. After validating both archived reports, delete the pilot fixtures and machinery: benchmark evidence/gold/query/task/schema files, every dedicated `scripts/graphify-context-*` harness/scoring file, and the exact dedicated external Graphify benchmark cache. Remove any other pilot-only executable artifact whose ownership is unambiguous.
7. Remove stale Graphify MCP references outside the archived `README.md`, `results.md`, `pilot-audit.json`, and these prompt documents. Do not erase the historical names from the archive.
...
Stop the affected mutation and ask the user before proceeding if artifact provenance is insufficient, independently scored results cannot be represented without choosing one as authoritative, a path cannot be proven to belong to the dedicated benchmark, or a shared configuration cannot be edited without affecting unrelated tools. Do not delete raw evidence until both retained reports validate, and do not declare success while a configured or runnable Graphify MCP surface remains.
...
2. **Pilot archive** — retained files, key provenance, and validation of their values.
