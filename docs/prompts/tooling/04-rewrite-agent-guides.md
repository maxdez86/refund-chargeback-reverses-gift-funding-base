...
You are authorized to edit only the five agent guides and the Claude import shims described below. Preserve unrelated work, settings, and the existing index exactly as found. Do not unstage or otherwise alter preceding-prompt changes that the user has already staged; leave only this prompt's guide/shim changes unstaged. Do not commit, push, create or publish a branch or pull request, modify production-promotion integration tests, or edit `docs/prompts/tooling/`.
...
- Prompt 3 should have added working `graphify:doctor`, `graphify:build`, and `graphify:query` commands. Revalidate their exact interface from `package.json`, implementation, tests, and `--help` before documenting them — including the **calibrated default query budget**, expected to be 12,000 tokens, which Prompt 3 selected empirically and which is deliberately not the pilot's 2,000-token figure.
- Prompt 3 deliberately builds a local code-only graph with `--code-only --no-cluster`. Documentation and other semantic inputs are not in the graph: Graphify-classified Markdown, YAML, HTML, images, and similar files are excluded by `--code-only`, while CSS is unsupported by the current parser. Do not imply that Graphify can discover or verify content in those formats. Do not cite the archived pilot's accuracy, token, or cost numbers as expected behaviour for the wrapper; although that pilot was also code-only, it used Graphify 0.9.39 with the MCP extra, an older repository state, a different ignore policy, and a different query budget.
...
1. Record `git status --short`, distinguish staged from unstaged paths, and preserve all preceding changes in their existing index/working-tree state. Do not stage, unstage, or rewrite preceding-prompt changes.
...
3. Graphify CLI is optional and experimental. Invoke it only through the repository's `pnpm graphify:*` wrapper commands, and only as a fast hypothesis generator for unfamiliar relationships spanning multiple layers of supported code.
...
5. Use native search—not Graphify—as the authoritative method for completeness/enumeration, exact ownership, security-sensitive flows, configuration values, contract consumers, reference/rename plans, documentation, and generated, unsupported, or code-only-excluded formats. In particular, use native search and focused reads for Markdown, YAML, HTML, CSS, images/media references, and other inputs absent from the code-only graph.
6. Fall back to native search when a graph is stale, rebuilt unsuccessfully, or missing expected results. Missing output may mean the relationship was not extracted or the file format was excluded, not that the symbol or behavior is absent. Treat truncation and any `INFERRED` or `AMBIGUOUS` relationship as a signal that the result is a partial hypothesis requiring native confirmation before use — not as a reason to discard the result outright. Never increase the budget merely to avoid verification.
7. Document the wrapper's calibrated default query budget as the value Prompt 3 actually selected and recorded, expected to be **12,000 tokens**. Read the value from the wrapper implementation and help output; if it differs, document the implementation value and report the drift. **Do not write 2,000 tokens as current guidance**. The pilot's frozen calibration truncated at 2,000 on every calibrated query, so that figure is historical and must not be reintroduced as guidance. Building/rebuilding is on demand through the wrapper and uses local `--code-only --no-cluster` extraction with no LLM backend; do not register MCP or run upstream assistant-skill installers.
...
- **API:** optional for unfamiliar flows crossing handlers, domain services, persistence, vendors, contracts, or frontend boundaries. Exact webhook/security behavior, configuration, consumers, completeness, and YAML/documentation evidence remain native-source tasks.
- **Web:** optional for unfamiliar flows crossing routes/components, hooks/services, contracts, API handlers, or deployment boundaries. Exact UI behavior, accessibility, configuration, consumers, completeness, CSS, HTML/documentation, and media references remain native-source tasks.
...
- Confirm the guides describe the wrapper as code-only and local, state that it uses `--code-only --no-cluster` with no LLM backend, and do not claim coverage of Markdown, YAML, HTML, CSS, images, or other excluded/unsupported formats.
- Review Graphify wording for all required native-verification categories and fallback conditions: completeness, ownership, security, configuration, consumers, documentation, excluded/unsupported formats, rename/reference tasks, stale, failure, truncation, missing output, `INFERRED`, and `AMBIGUOUS`.
...
- Review the final diff for unrelated changes, confirm none of Prompt 4's guide/shim changes were staged, and confirm any pre-existing staged paths remain staged and otherwise untouched.

## Required final report

Report these headings:

1. **Instruction design** — native-search default, experimental code-only Graphify boundary, excluded-format fallback, and source-verification policy.
...
6. **Working tree** — Prompt 4's relevant unstaged paths, any preserved pre-existing staged paths, and confirmation that Prompt 4 staged nothing and nothing was committed, pushed, or published.
