...
// apps/api/src/functions/payments-create/handler.ts
var handler_exports = {};
...
// apps/api/src/domain/payment-service.ts
var import_node_crypto2 = require("node:crypto");

// packages/contracts/src/admin.ts
var AdminGuestExportRowSchema = external_exports.object({
  householdId: external_exports.string(),
  guestId: external_exports.string(),
  invitationCode: external_exports.string(),
  guestName: external_exports.string(),
  phoneNumber: external_exports.string().optional(),
  rsvpStatus: external_exports.string(),
  allowedPlusOnes: external_exports.number().int().nonnegative()
});

// packages/contracts/src/guest.ts
var RsvpStatusSchema = external_exports.enum(["pending", "attending", "declined"]);
var GuestProfileSchema = external_exports.object({
  invitationCode: external_exports.string().min(4),
  householdId: external_exports.string().min(1),
  guestId: external_exports.string().min(1),
  guestName: external_exports.string().min(1),
  phoneNumber: external_exports.string().min(8).optional(),
  allowedPlusOnes: external_exports.number().int().nonnegative(),
  rsvpStatus: RsvpStatusSchema,
  dietaryNotes: external_exports.string().optional()
});

// packages/contracts/src/payments.ts
var PaymentMethodSchema = external_exports.enum(["PIX", "CREDIT_CARD"]);
var PaymentStatusSchema = external_exports.enum([
  "CREATED",
  "AWAITING_PAYMENT",
  "PROCESSING",
  "CONFIRMED",
...
var PaymentPayerSchema = external_exports.object({
  name: external_exports.string().min(2).max(120),
  email: external_exports.string().email().max(255),
  cpf: external_exports.string().min(11).max(18),
  phone: external_exports.string().min(8).max(20).optional()
});
var CreatePaymentRequestSchema = external_exports.object({
  giftId: external_exports.string().min(1),
  quantity: external_exports.number().int().positive().max(100).optional(),
  paymentMethod: PaymentMethodSchema,
  payer: PaymentPayerSchema
});
var PaymentGiftSummarySchema = external_exports.object({
  id: external_exports.string().min(1),
  name: external_exports.string().min(1),
  fractional: external_exports.boolean(),
  quantity: external_exports.number().int().positive(),
  unitAmountCents: external_exports.number().int().positive().nullable(),
  amountCents: external_exports.number().int().positive()
});
var PaymentPixDetailsSchema = external_exports.object({
  copyPaste: external_exports.string().min(1),
  qrCodeBase64: external_exports.string().min(1),
  expiresAt: external_exports.string().datetime().optional()
});
var PaymentSummarySchema = external_exports.object({
  paymentId: external_exports.string().min(1),
  paymentMethod: PaymentMethodSchema,
  status: PaymentStatusSchema,
  amountCents: external_exports.number().int().positive(),
  currency: external_exports.literal("BRL"),
  gift: PaymentGiftSummarySchema,
  invoiceUrl: external_exports.string().url().optional(),
  pix: PaymentPixDetailsSchema.optional(),
  createdAt: external_exports.string().datetime(),
  updatedAt: external_exports.string().datetime(),
  confirmedAt: external_exports.string().datetime().optional(),
  receivedAt: external_exports.string().datetime().optional()
});
var CreatePaymentResponseSchema = external_exports.object({
  ok: external_exports.literal(true),
  payment: PaymentSummarySchema
});
var GetPaymentResponseSchema = external_exports.object({
  ok: external_exports.literal(true),
  payment: PaymentSummarySchema
});

// packages/contracts/src/registry.ts
var RegistryCheckoutRequestSchema = external_exports.object({
  guestId: external_exports.string().min(1),
  invitationCode: external_exports.string().min(4),
  amountInCents: external_exports.number().int().positive(),
  currency: external_exports.string().length(3),
  message: external_exports.string().max(500).optional()
});
var RegistryCheckoutResponseSchema = external_exports.object({
  ok: external_exports.literal(true),
  checkoutUrl: external_exports.string().url(),
  contributionId: external_exports.string().min(1)
});

// packages/contracts/src/rsvp.ts
var RsvpGuestAnswerSchema = external_exports.object({
  guestId: external_exports.string().min(1),
  status: RsvpStatusSchema,
  mealPreference: external_exports.string().min(1).optional(),
  note: external_exports.string().max(500).optional()
});
var RsvpSubmissionRequestSchema = external_exports.object({
  invitationCode: external_exports.string().min(4),
  householdId: external_exports.string().min(1),
  submittedBy: external_exports.string().min(1),
  guestResponses: external_exports.array(RsvpGuestAnswerSchema).min(1),
  attendingGuestCount: external_exports.number().int().nonnegative(),
  note: external_exports.string().max(500).optional()
});
var RsvpSubmissionResponseSchema = external_exports.object({
  ok: external_exports.literal(true),
  invitationCode: external_exports.string(),
  householdId: external_exports.string(),
  status: RsvpStatusSchema,
...
var StripeWebhookEventSchema = external_exports.object({
  ...
  contributionId: external_exports.string().optional()
});
var WhatsappWebhookEventSchema = external_exports.object({
  provider: external_exports.literal("whatsapp"),
  eventId: external_exports.string().min(1),
  phoneNumber: external_exports.string().min(8),
  messageType: external_exports.string().min(1),
  receivedAt: external_exports.string()
});

// packages/config/src/gifts.ts
function single(id, name, priceInBrl) {
  return {
    id,
    name,
...
function fractional(id, name, priceInBrl) {
  const partValueCents = 5e3;
  return {
    id,
    name,
    totalValueCents: priceInBrl * 100,
...
var PAYMENT_GIFTS = [
  ...
  fractional("g-armario", "Arm\xE1rio de Cozinha", 1749),
  single("g-aspirador", "Aspirador", 139),
  single("g-balde", "Balde Retr\xE1til 10L", 69),
  single("g-batedeira", "Batedeira", 79),
  fractional("g-cama", "Cama", 1199),
  single("g-edredom", "Edredom King", 136),
  single("g-escorredor", "Escorredor de Lou\xE7a", 100),
  fractional("g-fogao", "Fog\xE3o", 1e3),
  fractional("g-guarda-roupa", "Guarda-roupa", 3e3),
  single("g-ferramentas", "Jogo de Ferramentas", 99),
  fractional("g-pratos", "Jogo de Pratos 12 Pe\xE7as", 331),
  single("g-talheres", "Jogo de Talheres", 178),
  single("g-xicaras", "Jogo de X\xEDcaras", 188),
  single("g-toalhas-rosto", "Kit 4 Toalhas de Rosto", 65),
  fractional("g-lava-seca", "Lava e Seca 11kg", 2900),
  fractional("g-lava-loucas", "Lava-lou\xE7as", 1900),
  ...
  single("g-purificador", "Purificador de \xC1gua", 169),
  fractional("g-refrigerador", "Geladeira Brastemp", 2960),
  fractional("g-sofa", "Sof\xE1", 1482),
  single("g-steamer", "Steamer", 141),
  single("g-travesseiros", "Travesseiros", 59)
];
var PAYMENT_GIFTS_BY_ID = new Map(PAYMENT_GIFTS.map((gift) => [gift.id, gift]));
...
// apps/api/src/services/asaas/client.ts
function parseSecretString(value) {
  if (!value) {
    return "";
  }
  try {
...
var AsaasClient = class {
  apiBaseUrl = getEnv().asaasApiBaseUrl;
  apiSecretArn = getEnv().asaasApiSecretArn;
  async findCustomerByCpf(cpfCnpj) {
  ...
  async listPaymentsByExternalReference(externalReference) {
    const response = await this.request({
      path: "/payments",
  ...
  async request({ method = "GET", path, body, query }) {
    ...
    if (!response.ok) {
      const message = String(parsed.errors ?? parsed.message ?? `Asaas request failed with status ${response.status}.`);
...
var dynamoDbDocumentClient = import_lib_dynamodb.DynamoDBDocumentClient.from(client2, {
  marshallOptions: {
...
function resolveGiftSelection(gift, quantity) {
  if (!gift.fractional) {
...
function toBrlDecimal(valueInCents) {
  return valueInCents / 100;
}
var PaymentService = class {
  constructor(repository = new PaymentRepository(), asaasClient = new AsaasClient()) {
    this.repository = repository;
    this.asaasClient = asaasClient;
  }
  async createPayment(request, idempotencyKeyHeader) {
    ...
    const idempotencyKey = idempotencyKeyHeader?.trim() || (0, import_node_crypto2.randomUUID)();
    ...
    const reservedPaymentId = (0, import_node_crypto2.randomUUID)();
    ...
    const payment = {
      ...
      invoiceUrl: parsed.paymentMethod === "CREDIT_CARD" ? asaasPayment.invoiceUrl : void 0,
      createdAt: now,
      updatedAt: now
    };
  ...
  async resolveExistingPayment(paymentId, paymentMethod) {
  ...
  async findOrCreateCustomer(request, normalizedCpf, paymentId) {
